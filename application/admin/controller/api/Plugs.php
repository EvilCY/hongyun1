...
class Plugs extends Controller
{
    ...
    public function upload()
    {
        ...
        // $name = File::name($this->request->post('md5'), $ext, '', 'strtolower');
        $name = date('Ym').'/'.md5($file->getPathname()).'.'.$this->ext;
        // Token 验证
    ...
    public function plupload()
    {
        ...
        if ($file->checkImgs()&&!$file->checkSize(5242880)) {
            return json(['uploaded' => false, 'error' => ['message' => '图片文件超过大小限制：5M！']]);
        }
        if ($file->checkMp4()&&!$file->checkSize(104857600)) {
            return json(['uploaded' => false, 'error' => ['message' => '视频文件超过大小限制：100M！']]);
        }
        $this->safe = $this->getUploadSafe();
        $this->uptype = $this->getUploadType();
